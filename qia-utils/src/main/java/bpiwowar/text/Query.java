package bpiwowar.text;

import bpiwowar.collections.Aggregator;
import bpiwowar.collections.DefaultMap;
...
import bpiwowar.utils.iterators.IntersectionIterator;
 ...
 *
 * Can compute interesting things with it: - ...
 *
 * @author bpiwowar
 */
public class Query {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Normalised query: " + query);
     ...
     */
    public static Similarities ngramSimilarties(int n, Query q1, Query q2) {
        ...
        DefaultMap<String, MutableInteger> maps[] = new DefaultMap[2];

        double[] l2 = new double[2];
        int[] l = {0, 0};

        for (int k = 0; k < 2; k++) {
            maps[k] = new DefaultMap<String, MutableInteger>(TreeMap.class,
                MutableInteger.class);
            for (int i = 0; i <= s[k].length() - n; i++) {
                maps[k].get(s[k].substring(i, i + n)).value++;
                l[k]++;
            }

            for (MutableInteger m : maps[k].values())
                l2[k] += m.value * m.value;

            l2[k] = Math.sqrt(l2[k]);
        }

        // And then compute
        IntersectionIterator<String, MutableInteger> iterator = new IntersectionIterator<String, MutableInteger>(
            MutableInteger.class, maps);
        while (iterator.hasNext()) {
            Pair<String, MutableInteger[]> pair = iterator.next();
            MutableInteger[] values = pair.getSecond();
            similarities.cosine += values[0].value * values[1].value;
            similarities.inclusion[0] += Math.min(values[0].value,
                values[1].value);
            similarities.inclusion[1] += Math.min(values[0].value,
                values[1].value);
        }
     ...
     * @return A structure containing words belonging to the three possible sets (only Q1, only Q2, or both queries)
     */
    static final WordDifferences computeWordDifferences(Query q1, Query q2) {
        // Get the query terms
