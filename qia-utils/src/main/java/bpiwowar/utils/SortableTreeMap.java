...
/**
 * Red-Black tree based implementation of the <tt>SortedMap</tt> interface. This class guarantees that the map will be
 * in ascending key order, sorted according to the <i>natural order</i> for the key's class (see
 ...
 * operations. Algorithms are adaptations of those in Cormen, Leiserson, and Rivest's <I>Introduction to
 * Algorithms</I>.
 * <p>
 *
 ...
 * is defined in terms of the equals operation, but a map performs all key comparisons using its <tt>compareTo</tt> (or
 * <tt>compare</tt>) method, so two keys that are deemed equal by this method are, from the standpoint of the sorted
 * map, equal. The behavior of a sorted map <i>is</i> well-defined even if its ordering is inconsistent with equals; it
 ...
 * access a map concurrently, and at least one of the threads modifies the map structurally, it <i>must</i> be
 * synchronized externally. (A structural modification is any operation that adds or deletes one or more mappings;
 * merely changing the value associated with an existing key is not a structural modification.) This is typically
 * accomplished by synchronizing on some object that naturally encapsulates the map. If no such object exists, the map
 * should be "wrapped" using the <tt>Collections.synchronizedMap</tt> method. This is best done at creation time, to
 ...
 * modification, the iterator fails quickly and cleanly, rather than risking arbitrary, non-deterministic behavior at an
...
public class SortableTreeMap<K, V> extends AbstractMap<K, V> implements
     ...
     * elements <tt>k1</tt> and <tt>k2</tt> in the map. If the user attempts to put a key into the map that violates
     * this constraint (for example, the user attempts to put a string key into a map whose keys are integers), the
     * <tt>put(Object key, Object value)</tt> call will throw a <tt>ClassCastException</tt>.
     *
     * @see Comparable
     */
    public SortableTreeMap() {
    }

    /**
     * Constructs a new, empty map, sorted according to the given comparator. All keys inserted into the map must be
     * <i>mutually comparable</i> by the given comparator: <tt>comparator.compare(k1, k2)</tt> must not throw a
     * <tt>ClassCastException</tt> for any keys <tt>k1</tt> and <tt>k2</tt> in
     * the map. If the user attempts to put a key into the map that violates this constraint, the <tt>put(Object key,
     ...
     */
    public SortableTreeMap(Comparator<? super K> c) {
        this.comparator = c;
    }
     ...
     */
    public SortableTreeMap(SortedMap<K, ? extends V> m) {
     ...
     * of <tt>null</tt> does not <i>necessarily</i> indicate that the map contains no mapping for the key; it's also
     * possible that the map explicitly maps the key to <tt>null</tt>. The <tt>containsKey</tt> operation may be used to
    ...
    public void setComparator(Comparator<? super K> comparator) {
        // Do not do anything if the comparator does not change
        if (comparator == this.comparator
            || (comparator != null && comparator.equals(this.comparator)))
            return;

        // Sort again the tree: we don't have to balance anything, we just
        this.comparator = comparator;

        if (root != null) {
            Entry<K, V> oldRoot = root;
            clear();
            recursiveInsert(oldRoot);
        }
    }

    private void recursiveInsert(Entry<K, V> node) {
        if (node.left != null) {
    ...
    /*
     * static final private <K, V> void swap(Entry<K, V> node1, Entry<K, V>
     * node2) { K key = node1.key; V value = node1.value; node1.key = node2.key;
     * node1.value = node2.value; node2.key = key; node2.value = value; }
     *
     * // multi-swap: with multiple nodes, swap each with the next & continue
     * static final private <K, V> void rotateLeft(Entry<K, V>... nodes) { K key
     * = nodes[0].key; V value = nodes[0].value; for (int i = 0; i <
     * nodes.length - 1; i++) { nodes[i].key = nodes[i + 1].key; nodes[i].value
     * = nodes[i + 1].value; } nodes[nodes.length - 1].key = key;
     * nodes[nodes.length - 1].value = value; }
     *
     * // Compare node1 and node2, and swap them iff node1 < node2 // Return
     * true if a swap occurred private boolean compareAndSwap(Entry<K, V> node1,
     * Entry<K, V> node2) { if (node1 != null && node2 != null) { int comp =
     * compare(node1.key, node2.key); if (comp > 0) { swap(node1, node2);
     * System.out.format("Swapped: %d < %d%n", node1.key, node2.key);
     * printTree(System.out); } else {
     * System.out.format("NOT swapped: %d < %d%n", node1.key, node2.key); } }
     * return false; }
     *
     * private Pair<Entry<K,V>,Entry<K,V>> sort2(Entry<K,V> node) {
     *
     * Pair<Entry<K,V>,Entry<K,V>> left = sort2(node.left);
     * Pair<Entry<K,V>,Entry<K,V>> right = sort2(node.right); return null;
     *
     * }
     *
     * private boolean sort(Entry<K, V> node) { boolean changed = false;
     *
     * // Handle the case where one or both children are null if (node.left ==
     * null) { if (node.right == null) return false; return compareAndSwap(node,
     * node.right); } else if (node.right == null) { return
     * compareAndSwap(node.left, node); }
     *
     * // We have a node with l & right if (compare(node.left.key, node.key) >
     * 0) { // we have l > n
     *
     * if (compare(node.key, node.right.key) > 0) { // case l > n > r : we swap
     * l and r swap(node.left, node.right); } else { // case l > n and r > n: we
     * compare l and r if (compare(node.left.key, node.right.key) > 0) { // case
     * l > r > n: we swap all rotateLeft(node, node.right, node.left); } else {
     * // case r > l > n swap(node.left, node); } }
     *
     * changed = true; } else { // we have l < n if (compare(node.key,
     * node.right.key) > 0) { // we have n > l & n > r if
     * (compare(node.left.key, node.right.key) > 0) { // case n > l > r [left
     * should be the node, the node should // be right, the right should be
     * left] rotateLeft(node, node.left, node.right); } else { // case n > r > l
     * swap(node, node.right); } changed = true; } else { // l < n < r: no
     * change! } }
     *
     * System.out.format("Node changed to: (%s < %s < %s)%n", node.left.key,
     * node.key, node.right.key); printTree(System.out);
     *
     * // Try to exchange subtrees changed |= sort(node.left); changed |=
     * sort(node.right);
     *
     * // Check that we don't have a local stable order by checking the grand
     * child (left/right) if ((node.left.right != null) && (compare(node.key,
     * node.left.right.key) < 0)) { changed = true; swap(node, node.left.right);
     * System.out.format("Changed grand-child (left/right) with us: %s < %s%n",
     * node.left.right.key, node.key); printTree(System.out); } // The same with
     * right/left (which must be > to us) if ((node.right.left != null) &&
     * (compare(node.key, node.right.left.key) > 0)) { changed = true;
     * swap(node, node.right.left);
     * System.out.format("Changed grand-child (left/right) with us: %s < %s%n",
     * node.right.left.key, node.key); printTree(System.out); }
     *
     * return changed; }
     */
    ...
    static private <K, V> void printTree(final PrintStream out, Entry<K, V> node) {
        if (node == null)
            out.print('�');
        else {
    ...
    public static void main(String[] args) {
        Comparator<Integer> c = new Comparator<Integer>() {
            public int compare(Integer o1, Integer o2) {
                return Integer.signum(o2 - o1);
            }
        };
        SortableTreeMap<Integer, Integer> stm = new SortableTreeMap<Integer, Integer>(
            c);

        for (int i = 1; i < 100; i++)
            stm.put(i, i);

        for (Map.Entry<Integer, Integer> e : stm.entrySet()) {
            System.out.format("%d (%d)%n", e.getKey(), e.getValue());
        }
        stm.printTree(System.out);

        System.out.println("===== Changing comparator");
        stm.setComparator(null);
     ...
     * map.
     * @throws NullPointerException if the given map is <tt>null</tt> or this map does not permit
     * <tt>null</tt> keys and a key in the specified map is
     * <tt>null</tt>.
     */
    public void putAll(Map<? extends K, ? extends V> map) {
        int mapSize = map.size();
        if (size == 0 && mapSize != 0 && map instanceof SortedMap) {
            Comparator<?> c = ((SortedMap<? extends K, ? extends V>)map)
                .comparator();
     ...
     * <tt>null</tt> return can also
     * indicate that the map previously associated <tt>null</tt> with the specified key.
     * @throws ClassCastException key cannot be compared with the keys currently in the map.
     * @throws NullPointerException key is <tt>null</tt> and this map uses natural order, or its comparator does not
     * tolerate <tt>null</tt> keys.
     */
    ...
    /**
     * Removes the mapping for this key from this SortableTreeMap if present.
     *
    ...
    /**
     * Removes all mappings from this SortableTreeMap.
     */
    public void clear() {
        modCount++;
        size = 0;
    ...
    /**
     * Returns a shallow copy of this <tt>SortableTreeMap</tt> instance. (The keys and values themselves are not
     * cloned.)
     *
     * @return a shallow copy of this Map.
     */
    public Object clone() {
        SortableTreeMap<K, V> clone = null;
        try {
            clone = (SortableTreeMap<K, V>)super.clone();
        }
    ...
    /**
     * This field is initialized to contain an instance of the entry set view the first time this view is requested. The
     * view is stateless, so there's no reason to create more than one.
     */
    ...
    /**
     * Returns a Set view of the keys contained in this map. The set's iterator will return the keys in ascending order.
     * The map is backed by this
     * <tt>SortableTreeMap</tt> instance, so changes to this map are reflected
     * in the Set, and vice-versa. The Set supports element removal, which removes the corresponding mapping from the
     * map, via the
     * <tt>Iterator.remove</tt>, <tt>Set.remove</tt>, <tt>removeAll</tt>,
     * <tt>retainAll</tt>, and <tt>clear</tt> operations. It does not support
     * the <tt>add</tt> or <tt>addAll</tt> operations.
     *
     * @return a set view of the keys contained in this SortableTreeMap.
     */
    public Set<K> keySet() {
        if (keySet == null) {
            keySet = new AbstractSet<K>() {
                ...
                public int size() {
                    return SortableTreeMap.this.size();
                }
                ...
                public boolean remove(Object o) {
                    int oldSize = size;
                    SortableTreeMap.this.remove(o);
                    return size != oldSize;
                }

                public void clear() {
                    SortableTreeMap.this.clear();
                }
            };
        }
        return keySet;
     ...
     * in the order that their corresponding keys appear in the tree. The collection is backed by this
     * <tt>SortableTreeMap</tt> instance, so changes to this map are reflected
     * in the collection, and vice-versa. The collection supports element removal, which removes the corresponding
     * mapping from the map through the
     ...
     * key order. Each element in the returned set is a <tt>Map.Entry</tt>. The set is backed by this map, so changes to
     * this map are reflected in the set, and vice-versa. The set supports element removal, which removes the
     * corresponding mapping from the SortableTreeMap, through the <tt>Iterator.remove</tt>,
     ...
     * empty.) The returned sorted map is backed by this map, so changes in the returned sorted map are reflected in
     * this map, and vice-versa. The returned sorted map supports all optional map operations.
     * <p>
     *
     ...
     * </pre>
     *
     * A similar technique can be used to generate an <i>open range</i> (which contains neither endpoint). The following
     * idiom obtains a view containing all of the key-value mappings in <tt>m</tt> whose keys are between
     ...
     * sorted map whose keys are strings. The following idiom obtains a view containing all of the key-value mappings in
     * <tt>m</tt> whose keys are less than or equal to
     * <tt>high</tt>:
     *
     * <pre>
     ...
     * @throws IllegalArgumentException if this map is itself a subMap, headMap, or tailMap, and
     ...
     * keys are strings. The following idiom obtains a view containing all of the key-value mappings in <tt>m</tt> whose
     * keys are strictly greater than <tt>low</tt>:
     *
     * <pre>
    ...
    private class SubMap extends AbstractMap<K, V> implements SortedMap<K, V>,
        ...
        public boolean containsKey(Object key) {
            return inRange((K)key) && SortableTreeMap.this.containsKey(key);
        }
        ...
        public V get(Object key) {
            if (!inRange((K)key))
                return null;
            return SortableTreeMap.this.get(key);
        }

        public V put(K key, V value) {
            if (!inRange(key))
                throw new IllegalArgumentException("key out of range");
            return SortableTreeMap.this.put(key, value);
        }
        ...
        public K firstKey() {
            SortableTreeMap.Entry<K, V> e = fromStart ? firstEntry()
        ...
        public K lastKey() {
            SortableTreeMap.Entry<K, V> e = toEnd ? lastEntry()
        ...
        private class EntrySetView extends AbstractSet<Map.Entry<K, V>> {
            ...
            public int size() {
                if (size == -1 || sizeModCount != SortableTreeMap.this.modCount) {
                    size = 0;
                    sizeModCount = SortableTreeMap.this.modCount;
                    Iterator<?> i = iterator();
                    while (i.hasNext()) {
                        size++;
                        i.next();
            ...
            public boolean contains(Object o) {
                ...
                if (!inRange(key))
                    return false;
                SortableTreeMap.Entry<K, V> node = getEntry(key);
                return node != null
    ...
    /**
     * SortableTreeMap Iterator.
     */
    private abstract class PrivateEntryIterator<T> implements Iterator<T> {
        private int expectedModCount = SortableTreeMap.this.modCount;
    ...
    /**
     * Compares two keys using the correct comparison method for this SortableTreeMap.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns the first Entry in the SortableTreeMap (according to the SortableTreeMap's key-sort function). Returns
     * null if the SortableTreeMap is empty.
     */
    ...
    /**
     * Returns the last Entry in the SortableTreeMap (according to the SortableTreeMap's key-sort function). Returns
     * null if the SortableTreeMap is empty.
     */
    ...
    /**
     * Save the state of the <tt>SortableTreeMap</tt> instance to a stream (i.e., serialize it).
     */
    ...
    /**
     * Reconstitute the <tt>SortableTreeMap</tt> instance from a stream (i.e., deserialize it).
     */
     ...
     *
     * It is assumed that the comparator of the SortableTreeMap is already set prior to calling this method.
     *
     ...
     * size fields of the SortableTreeMap are already set prior to calling this method. (It ignores both fields.)
     *
