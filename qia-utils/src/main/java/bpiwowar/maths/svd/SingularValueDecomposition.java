...
/**
 * For an <tt>m x n</tt> matrix <tt>A</tt> with <tt>m >= n</tt>, the singular value decomposition is an <tt>m x n</tt>
 * orthogonal matrix <tt>U</tt>, an
 ...
 * <P>
 * The singular value decomposition always exists, so the constructor will never fail. The matrix condition number and
 * the effective numerical rank can be computed from this decomposition.
 *
 * <p>
 * Implementation taken from Colt 1.2.0 and modified for faster incremental SVD
 *
 * </p>
 *
 * @author Colt team
 * @author B. Piwowarski
 */
final public class SingularValueDecomposition implements java.io.Serializable {
    ...
    public SingularValueDecomposition(double[] A, int m, int n) {
        this(A, m, n, true, true);
    }
     ...
     *
     * @param A A rectangular matrix (cols >= rows) stored in row by row format, i.e. cell (i, j) is in the cell i *
     * nbcols + j
     ...
     */
    public SingularValueDecomposition(double[] A, int m, int n, boolean wantu,
        boolean wantv) {
        if (m < n)
            throw new IllegalArgumentException(String.format(
                "Matrix must be rectangular: %dx%d matrix", m, n));
     ...
     * @return S
     */
    public DoubleMatrix2D getS(int rank) {
        DoubleMatrix2D S = new DiagonalDoubleMatrix(rank, rank);
    ...
    public DoubleMatrix2D getU() {
        ...
        final int p = Math.min(m + 1, n);
        DoubleMatrix2D U = new DenseDoubleMatrix2D(m, p);
        U.viewPart(0, 0, m, p).assign(this.U);
        return U;
    }
    ...
    public DoubleMatrix2D getV() {
        DoubleMatrix2D V = new DenseDoubleMatrix2D(this.V.length, this.V[0].length);
        V.assign(this.V);
        return V;
    }
