...
*/
package bpiwowar.random;

import cern.jet.random.engine.RandomEngine;
import cern.jet.random.sampling.RandomSamplingAssistant;

/**
 * Space and time efficiently computes a sorted <i>Simple Random Sample Without Replacement (SRSWOR)</i>, that is, a
 * sorted set of <tt>n</tt> random numbers from an interval of <tt>N</tt> numbers; Example: Computing <tt>n=3</tt>
 * random numbers from the interval <tt>[1,50]</tt> may yield the sorted random set <tt>(7,13,47)</tt>. Since we are
 * talking about a set (sampling without replacement), no element will occur more than once. Each number from the
 * <tt>N</tt> numbers has the same probability to be included in the <tt>n</tt> chosen numbers.
 *
 * <p><b>Problem:</b> This class solves problems including the following: <i>
 * Suppose we have a file containing 10^12 objects. We would like to take a truly random subset of 10^6 objects and do
 * something with it, for example, compute the sum over some instance field, or whatever. How do we choose the subset?
 * In particular, how do we avoid multiple equal elements? How do we do this quick and without consuming excessive
 * memory? How do we avoid slowly jumping back and forth within the file? </i>
 *
 * <p><b>Sorted Simple Random Sample Without Replacement (SRSWOR):</b>
 * What are the exact semantics of this class? What is a SRSWOR? In which sense exactly is a returned set "random"? It
 * is random in the sense, that each number from the <tt>N</tt> numbers has the same probability to be included in the
 * <tt>n</tt> chosen numbers. For those who think in implementations rather than abstract interfaces:
 * <i>Suppose, we have an empty list.
 * We pick a random number between 1 and 10^12 and add it to the list only if it was not already picked before, i.e. if
 * it is not already contained in the list. We then do the same thing again and again until we have eventually collected
 * 10^6 distinct numbers. Now we sort the set ascending and return it.</i>
 * <dt>It is exactly in this sense that this class returns "random" sets.
 * <b>Note, however, that the implementation of this class uses a technique orders of magnitudes better (both in time
 * and space) than the one outlined above.</b>
 *
 * <p><b>Performance:</b> Space requirements are zero. Running time is <tt>O(n)</tt> on average, <tt>O(N)</tt> in the
 * worst case.
 ...
 * </center>
 *
 * <p><b>Scalability:</b> This random sampler is designed to be scalable. In iterator style, it is able to compute and
 * deliver sorted random sets stepwise in units called <i>blocks</i>.
 * Example: Computing <tt>n=9</tt> random numbers from the interval <tt>[1,50]</tt> in 3 blocks may yield the blocks
 * <tt>(7,13,14), (27,37,42), (45,46,49)</tt>. (The maximum of a block is guaranteed to be less than the minimum of its
 * successor block. Every block is sorted ascending. No element will ever occur twice, both within a block and among
 * blocks.) A block can be computed and retrieved with method <tt>nextBlock</tt>. Successive calls to method
 * <tt>nextBlock</tt> will deliver as many random numbers as required.
 *
 * <p>Computing and retrieving samples in blocks is useful if you need very many random numbers that cannot be stored in
 * main memory at the same time.
 * For example, if you want to compute 10^10 such numbers you can do this by computing them in blocks of, say, 500
 * elements each. You then need only space to keep one block of 500 elements (i.e. 4 KB). When you are finished
 * processing the first 500 elements you call <tt>nextBlock</tt> to fill the next 500 elements into the block, process
 * them, and so on. If you have the time and need, by using such blocks you can compute random sets up to
 ...
 *
 * <p><b>Random number generation:</b> By default uses <tt>MersenneTwister</tt>, a very strong random number generator,
 * much better than <tt>java.util.Random</tt>.
 * You can also use other strong random number generators of Paul Houle's RngPack package. For example, <tt>Ranecu</tt>,
 * <tt>Ranmar</tt> and <tt>Ranlux</tt> are strong well analyzed research grade pseudo-random number generators with
 * known periods.
 *
 * <p><b>Implementation:</b> after J.S. Vitter, An Efficient Algorithm for Sequential Random Sampling,
 * ACM Transactions on Mathematical Software, Vol 13, 1987. Paper available <A HREF="http://www.cs.duke.edu/~jsv">
 * here</A>.
 *
...
public class RandomSampler extends cern.colt.PersistentObject {
     ...
     * @param low the interval to choose random numbers from is <tt>[low,low+N-1]</tt>. Hint: If <tt>low==0</tt>, then
     * random numbers will be drawn from the interval <tt>[0,N-1]</tt>.
	...
	*/
     ...
     * generator, much better than <tt>java.util.Random</tt>.
     * You can also use other strong random number generators of Paul Houle's RngPack package. For example,
     * <tt>Ranecu</tt>, <tt>Ranmar</tt> and <tt>Ranlux</tt> are strong well analyzed research grade pseudo-random number
    ...
    public static void sample(long n, long N, int count, long low, long[] values, int fromIndex,
        ...
        // Bug
        if (n == N) { // rare case treated quickly
            long val = low;
